...
void netsnmp_get_monotonic_clock(struct timeval* tv)
{
     ...
     * increases.
     */
    typedef ULONGLONG (WINAPI * pfGetTickCount)(void);
    static int s_initialized;
    static pfGetTickCount s_pfGetTickCount;
    uint64_t now64;

    if (!s_initialized) {
        HMODULE hKernel32 = GetModuleHandle("kernel32");
        s_pfGetTickCount =
            (pfGetTickCount) GetProcAddress(hKernel32, "GetTickCount");
        s_initialized = TRUE;
    }

    if (s_pfGetTickCount) {
        /* Windows Vista, Windows 2008 or any later Windows version */
        now64 = (*s_pfGetTickCount)();
    } else {
