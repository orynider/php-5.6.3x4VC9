...
//extern unsigned int if_nametoindex(val);
/* 
...
retry:
	...
	{
		char addr_str[17] = {0};
		inet_ntop32(AF_INET, addr, addr_str, sizeof(addr_str));
		php_error_docref(NULL TSRMLS_CC, E_WARNING,
...
int php_add4_to_if_index(struct in_addr *addr, php_socket *php_sock, unsigned *if_index TSRMLS_DC)
{
	...
	for (p = if_conf.ifc_buf;
		...
		if ((((struct sockaddr*)&cur_req->ifr_addr)->sa_family == AF_INET) &&
			...
			unsigned index_tmp;
			if ((index_tmp = php_if_nametoindex(cur_req->ifr_name)) == 0) {
#else
