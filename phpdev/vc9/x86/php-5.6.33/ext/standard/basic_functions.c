...
//#if (_WIN32_WINDOWS <= 0x0600)
#ifdef HAVE_INET_NTOP
	PHP_RAW_NAMED_FE(inet_ntop32,		php_inet_ntop,						arginfo_inet_ntop)
#endif
#ifdef HAVE_INET_PTON
	PHP_RAW_NAMED_FE(inet_pton32,		php_inet_pton,						arginfo_inet_pton)
#endif
//#endif
...
/* INET_NTOP replaced by HAVE_INET_NTOP32 for Windows XP */
#include "phpws2_32.h"
...
/* Windows XP */ /* */
	if (!inet_ntop32(af, address, buffer, sizeof(buffer))) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "An unknown error occurred - inet_ntop not defined in php sapi");
		RETURN_FALSE;
	}
/* */
...
#ifdef HAVE_INET_PTOP
	ret = inet_pton32(af, address, buffer);
#else
	ret = inet_pton32(af, address, buffer);	
#endif
	if (ret <= 0) {
...
#ifdef HAVE_INET_PTON	
	RETURN_LONG(inet_pton32(AF_INET, addr, &ip));
//#endif
//#if (_WIN32_WINDOWS <= 0x0600)	
#else
...
//#if (_WIN32_WINDOWS <= 0x0600) 
#ifdef HAVE_INET_PTON
	if (inet_ntop32(AF_INET, &myaddr, str, sizeof(str))) {
		RETURN_STRING(str, 1);
	} else {
		RETURN_FALSE;
	}
