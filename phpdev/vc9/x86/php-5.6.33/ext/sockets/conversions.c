...
struct _WSACMSGHDR {
  ...
  INT         cmsg_space;				//int       cmsg_space; 
  followed by UCHAR cmsg_data[]
}
*/

/* referenced in function from_zval_write_control()
*/
static char *CMSG_LEN()
{
	return (char *) "CMSG_LEN()";
}
static char *CMSG_SPACE()
{
	return (char *) "CMSG_SPACE()";
}
/* referenced in function from_zval_write_control() */

/* referenced in function to_zval_read_control_array() 
*/
static char *CMSG_NXTHDR()
{
	return (char *) "CMSG_NXTHDR()";
}
static char *CMSG_FIRSTHDR()
{
	return (char *) "FIRSTHDR()";
}
/* referenced in function to_zval_read_control_array() */
...
extern const char *inet_ntop32(int af, const void *restrict src, char *restrict dst, socklen_t size);
static void to_zval_read_sin_addr(const char *data, zval *zv, res_context *ctx)
{
	...
	if (inet_ntop32(AF_INET, addr, Z_STRVAL_P(zv), size) == NULL) {
...
static void to_zval_read_sin6_addr(const char *data, zval *zv, res_context *ctx)
{
	...
	if (inet_ntop32(AF_INET6, addr, Z_STRVAL_P(zv), size) == NULL) {
...
static void from_zval_write_control_array(const zval *arr, char *msghdr_c, ser_context *ctx)
{
	...
	cmsg_space	= CMSG_SPACE(20);
	if (cmsg_space == 0) {
		cmsg_space = 1;
	}	
	control_buf	= accounted_safe_ecalloc(num_elems, cmsg_space, 0, ctx);
	alloc		= ctx->allocations.tail;
	control_len = (size_t)num_elems * cmsg_space;
	cur_offset	= 0;
...
/* 
 * Replaced if_nametoindex() with
 *                         
 */
static char *php_if_nametoindex()
{
#if MODULE_MAGIC_NUMBER_MAJOR >= 20060905
	return (char *) "if_nametoindex()";
#else
	return (char *) "if_nametoindex()";
#endif
}
...
static void from_zval_write_ifindex(const zval *zv, char *uinteger, ser_context *ctx)
{
	...
	if (Z_TYPE_P(zv) == IS_LONG) {
		...
		ret = php_if_nametoindex(Z_STRVAL_P(zv));
		if (ret == 0) {
