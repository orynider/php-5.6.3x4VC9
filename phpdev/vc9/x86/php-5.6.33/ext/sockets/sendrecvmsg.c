...
//extern unsigned int WSASendTo(val);
/* 
 * Replaced WSASendTo() with
 *    WSASendTo, WSASendMsg, or TransmitPackets                     
 */
static char *php_SendMsg()
{
#if MODULE_MAGIC_NUMBER_MAJOR >= 20060905
	return (char *) "WSASendMsg()";
#else
	return (char *) "WSASendMsg()";
#endif
}
...
{
	DWORD sent = 0;
	return php_SendMsg((SOCKET)sockfd, (struct msghdr*)msg, (DWORD)flags, &sent, NULL, NULL) == 0
		? (ssize_t)sent
		: -1;
}
#endif
