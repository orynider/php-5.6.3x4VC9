...
void fcgi_set_allowed_clients(char *ip)
{
	...
	if (ip) {
		...
		while (cur) {
			...
			if (inet_pton32(AF_INET, cur, &allowed_clients[n].sa_inet.sin_addr)>0) {
				allowed_clients[n].sa.sa_family = AF_INET;
				n++;
			} else if (inet_pton32(AF_INET6, cur, &allowed_clients[n].sa_inet6.sin6_addr)>0) {
...
const char *fcgi_get_last_client_ip() /* {{{ */
{
	...
	if (client_sa.sa.sa_family == AF_INET6
		&& IN6_IS_ADDR_V4MAPPED(&client_sa.sa_inet6.sin6_addr)) {
		return inet_ntop32(AF_INET, ((char *)&client_sa.sa_inet6.sin6_addr)+12, str, INET6_ADDRSTRLEN);
	}
#endif
	/* Ipv6 */
	if (client_sa.sa.sa_family == AF_INET6) {
