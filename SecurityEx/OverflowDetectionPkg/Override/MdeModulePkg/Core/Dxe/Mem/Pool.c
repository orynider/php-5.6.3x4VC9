  ...
  )
{
  ...
  if ((UINT32) MemoryType >= MEMORY_TYPE_OEM_RESERVED_MIN) {
    ...
    Pool = CoreAllocatePoolI (EfiBootServicesData, sizeof (POOL), FALSE, NULL);
    if (Pool == NULL) {
      return NULL;
    }
  ...
  )
{
  ...
  *Buffer = CoreAllocatePoolI (PoolType, Size, NeedGuard, AllocatedPages);
  ...
  )
{
  ...
  Status = CoreInternalAllocatePool (PoolType, Size, Buffer, NeedGuard, &AllocatedPages);
  if (!EFI_ERROR (Status)) {
    CoreUpdateProfile (
  ...
  )
{
  ...
  if (IsListEmpty (&Pool->FreeList[Index])) {
    ...
    //
    if (AllocatedPages != NULL) {
      *AllocatedPages = EFI_SIZE_TO_PAGES(Granularity);
    }
    NewPage = CoreAllocatePoolPages(PoolType, EFI_SIZE_TO_PAGES (Granularity), Granularity, NeedGuard);
    if (NewPage == NULL) {
      goto Done;
    }
